...
def validate_workflow_policy(contract: dict[str, Any], errors: list[str]) -> dict[str, set[str]]:
    ...
    for index, item in enumerate(raw_exceptions):
        ...
        if (
            ...
            or not scopes
            or not all(isinstance(scope, str) and bool(scope) and scope == scope.lower() for scope in scopes)
        ):
...
def _validate_action_reference(uses: str, step: dict[str, Any] | None, label: str, errors: list[str]) -> None:
    if uses.startswith("./"):
        return
...
def validate_workflows(
    ...
    for path in workflow_paths:
        ...
        if "permissions" not in document:
            ...
            top_scopes = _permission_write_scopes(document["permissions"], f"{rel}: top-level", errors)
        ...
        if isinstance(jobs, dict):
            for job_name, raw_job in jobs.items():
                if isinstance(raw_job, dict) and "permissions" in raw_job:
                    write_scopes.update(
                        _permission_write_scopes(raw_job["permissions"], f"{rel}: job {job_name!r}", errors)
                    )
        ...
        if "*" in write_scopes:
            ...
            if expected != write_scopes:
                errors.append(
                    f"{rel}: write scopes {sorted(write_scopes)} require an exact write_permission_exceptions entry"
                )
