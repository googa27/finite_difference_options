...
def _optional_import_try(node: ast.Try) -> bool:
    ...
    handlers_are_fallbacks = all(
        all(
            ...
            and (_metadata_assignment(item) or (isinstance(item.value, ast.Constant) and item.value.value is None))
            for item in handler.body
        )
        for handler in node.handlers
    )
    metadata_body = all(
        isinstance(item, (ast.Assign, ast.AnnAssign)) and _metadata_assignment(item) for item in node.body
    )
    return not node.orelse and not node.finalbody and handlers_are_fallbacks and (body_is_imports or metadata_body)
...
def marker_only_package(path: Path) -> bool:
    ...
    for candidate in path.rglob("*"):
        if ignored(candidate.relative_to(ROOT)) or not candidate.is_file() or candidate == init_path:
            continue
...
def runtime_branches(current: Path) -> list[tuple[str, int]]:
    ...
    direct = sum(path.suffix == ".py" and path.name != "__init__.py" for path in current.iterdir() if path.is_file())
    if direct:
...
def concentration(branch_counts: list[int]) -> tuple[bool, float, float]:
    ...
    if count == THREE_BRANCHES:
        triggered = largest >= THREE_BRANCH_MAX_SHARE and (
            effective <= THREE_BRANCH_MAX_EFFECTIVE or fraction <= THREE_BRANCH_MAX_EFFECTIVE_FRACTION
        )
        ...
        triggered = largest >= SIX_SEVEN_MAX_SHARE and fraction <= MANY_BRANCH_MAX_EFFECTIVE_FRACTION
    else:
        triggered = largest >= MANY_BRANCH_MAX_SHARE and fraction <= MANY_BRANCH_MAX_EFFECTIVE_FRACTION
...
def validate_directory(
    ...
    if not review_concentration or not counts or sum(counts) < limits.minimum_modules:
        return
    triggered, largest, effective = branch_review_metrics(counts, limits.minimum_branches, limits.two_branch_threshold)
