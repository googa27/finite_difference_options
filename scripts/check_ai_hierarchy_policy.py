...
def init_implementation(path: Path) -> list[str]:
    ...
    for node in tree.body:
        if isinstance(node, ast.ClassDef) or (
            isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)) and node.name not in ALLOWED_INIT_FUNCTIONS
        ):
    ...
    for path in sorted((*workflows.glob("*.yml"), *workflows.glob("*.yaml"))):
        ...
        for index, line in enumerate(lines):
            ...
            if CHECKOUT_PATTERN.search(action) and not checkout_is_nonpersistent(lines, index):
                errors.append(f"checkout credentials persist without an exact policy exception: {rel}:{index + 1}")
