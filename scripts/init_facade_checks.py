...
def _static_metadata_assignment(node: ast.Assign | ast.AnnAssign) -> bool:
    return _metadata_assignment(node) and not any(isinstance(child, ast.Call) for child in ast.walk(node.value))
...
def _optional_import_try(node: ast.Try) -> bool:
    ...
    handlers_are_fallbacks = all(
        all(
            ...
            and (
                _static_metadata_assignment(item) or (isinstance(item.value, ast.Constant) and item.value.value is None)
            )
