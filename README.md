...
### arXiv-Lab FD oracle fixture

A deterministic, public European-call Black-Scholes fixture is published for external benchmark consumers (including arXiv-Lab). The JSON bundle is self-contained and plain JSON, so it does not require importing project internals.

- Problem spec and manifest metadata are in `problem_spec`
- Result export is in `result_export` with explicit:
  - `time_axis` orientation (`decreasing` because the legacy PDE solver reports
    valuation at the terminal index),
  - `grid` + `domain` metadata,
  - typed boundary specs,
  - `solution`, `reference`, `errors`, and no-arbitrage checks.

The fixture file is generated by:

```bash
uv run python - <<'PY'
from src.validation.black_scholes_parity import export_public_black_scholes_fixture_json
export_public_black_scholes_fixture_json(path="tests/fixtures/arxiv_lab_bs_oracle_v1.json")
PY
```
...
```text
{"fixture_id": "public-synthetic.black-scholes-call.v0", "schema_version": "arxiv-lab/fd-oracle-fixture/v0"}
```

This is consumed directly as a JSON artifact by downstream benchmarks:

```bash
python - <<'PY'
import json
payload = json.loads(open('tests/fixtures/arxiv_lab_bs_oracle_v1.json').read())
print(payload['problem_spec']['problem_id'])
print(payload['result_export']['solution']['price'])
PY
```
