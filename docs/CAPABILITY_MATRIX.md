...
| Callable fixed-rate bond cash-flow and Bermudan call-date reference route | `experimental` | none | `CallableBondPDEModel` now requires explicit cash-flow/coupon and call schedules with call dates strictly before final maturity, applies issuer exercise only on contractual dates, keeps same-date coupons outside the call cap, supports clean/dirty call settlement with coupon-date clean calls treated ex-coupon, spans remaining maturity from settlement, and exposes exercise diagnostics. It is a reference backward-induction route, not a production Hull-White/QuantLib-validated callable-bond engine. |
