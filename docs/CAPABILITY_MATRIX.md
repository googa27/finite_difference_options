...
| Callable fixed-rate bond cash-flow and Bermudan call-date reference route | `experimental` | none | `CallableBondPDEModel` now requires explicit cash-flow/coupon and call schedules, applies issuer exercise only on contractual dates, supports clean/dirty call settlement, and exposes exercise diagnostics. It is a reference backward-induction route, not a production Hull-White/QuantLib-validated callable-bond engine. |
