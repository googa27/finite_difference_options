...
| Haircut backend adapter / solver contract screening | `validated` | `QPS-VANILLA-CALL-V0`, `BS-CALL-PARITY-V0` | Transitional `finite_difference_options.integrations.haircut_backend:create_backend` factory exposes identity, capability manifest, fail-closed screening, and public-synthetic Black-Scholes solve evidence without importing Haircut/PDP/API/UI stacks. Numerical execution requires the payload to match `public_black_scholes_problem_spec()` exactly plus `privacy_class=public_synthetic`; label-compatible Haircut-origin fixtures are screened but fail closed until a matching executable oracle fixture exists. Clean-wheel entry-point publication remains tied to the package-namespace work in #51/#52. |
