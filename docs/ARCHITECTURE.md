...
Issue #142 adds the executable `fd-bs-001` verification artifact runner at `fd-options validation run-benchmark fd-bs-001 --out <path>`. It reuses the exact compiled Black-Scholes native FD route and emits independently recomputable evidence: analytical q-aware price/Delta/Gamma gates, three-level spatial refinement, temporal convergence against a same-space high-time-step reference, payoff/algebraic/boundary residual norms, separately manufactured PDE consistency residuals and observed order, no-arbitrage checks, the boundary schedule actually applied, independent operator-sign/source/reaction/boundary perturbation failures with recomputed pass/fail thresholds, installed package provenance, and request/config/convention/result/status/evidence hashes. `validate_fd_bs_verification_bundle(...)` recomputes all hashes, recomputes `evidence.status`, and reruns the numerical truth before accepting the artifact, so residual convergence, rehashed false rows, or stored booleans alone cannot qualify a run as verified.
