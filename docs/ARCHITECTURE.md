...
| American exercise discussed without an obstacle solve | European PDE results could be mislabeled as American | #66 adds explicit American/Bermudan option contracts, a 1D Black-Scholes projected-SOR LCP reference route, primal/dual/complementarity diagnostics, active-set/exercise-boundary evidence, and fail-closed nonconvergence. Multidimensional American/ADI remains gated until separate work-precision evidence exists. |
...
Issue #58 adds `finite_difference_options.validation.greek_derivative_gates` and the executable registry row `FD-GREEKS-VALIDATION-V0`. The PR gate runs a 12-case Black-Scholes matrix over in/at/out-of-the-money spots, short/long maturities and low/high volatility on strike-centered nonuniform grids, checks Delta/Gamma against closed-form derivatives, requires material refinement improvement, bounds strike-on-node versus half-cell shifted sensitivity, verifies expiry-kink rejection and reuses the Rannacher Gamma roughness check. `scripts/run_greek_derivative_validation.py` writes `finite-difference-greek-validation/v0` JSON artifacts; blocking CI uploads the PR artifact and `.github/workflows/derivative-validation.yml` runs the broader scheduled matrix.
...
Black-Scholes nonuniform-Greek evidence is split between `FD-GREEKS-NONUNIFORM-V0` for local stencil/requested-coordinate diagnostics and `FD-GREEKS-VALIDATION-V0` for the convergence/stability artifact gate required before changing maturity claims.
...
Issue #141 adds `finite_difference_options.integrations.compiled_pde_adapter` and the `fd-options qps screen/solve` CLI as the compiled `pde_ir.v0` consumption seam for VQPW. The seam consumes only serialized public-synthetic fixtures, imports no `financial_problem_formulations` source tree, recomputes and preserves the source IR canonical hash and compiled-operator hash, carries measure/numeraire/units/time/boundary evidence into the result bundle, and executes only the exact 1D European Black-Scholes/fixed-price proxy-compatible public route through a compiled-PDE-specific native FD route. That route maps the validated coefficients, domain/grid, theta controls and explicit time-dependent far-field boundary schedule into native matrices and records the applied schedule in evidence; it does not call the legacy parity fixture, infer rate from `model.mu`, or copy boundary metadata as a substitute for solver provenance. Mutated/private payloads, unsupported boundary/dimension/exercise/output requests, hash drift, and compiler-schema drift fail before grid, operator, or solver allocation.

Issue #175 extracts the compiled adapter's immutable result DTOs and identity vocabulary into `integrations/_compiled_pde_contracts.py`, and strict JSON plus source/compiler/solver-plan validation into `integrations/_compiled_pde_validation.py`. The public `compiled_pde_adapter` module retains fixture loading, screening and solve orchestration. All existing function/class bodies and numerical routes are unchanged; public class module names remain stable for trusted historical DTO pickles. The same prerequisite extracts grid-level/refinement/residual measurements from `validation/fd_verification.py` into the existing `validation/fd_evidence/grid_metrics.py` responsibility, preserving the public verification functions and legacy private aliases. This removes the 782-line adapter exception without increasing a maintainability ceiling. The captured c85b403 wheel contract in `tests/fixtures/compiled_pde_adapter_v0_contract.json` fixes public signatures, screening JSON and historical pickle identities; its provenance is documented in `docs/COMPILED_ADAPTER_EXTRACTION.md`.

Issue #142 adds the executable `fd-bs-001` verification artifact runner at `fd-options validation run-benchmark fd-bs-001 --out <path>`. It reuses the exact compiled Black-Scholes native FD route and emits independently recomputable evidence: analytical q-aware price/Delta/Gamma gates, three-level spatial refinement, temporal convergence against a same-space high-time-step reference, payoff/algebraic/boundary residual norms, separately manufactured PDE consistency residuals with an h²-scaled cap and observed order, no-arbitrage checks, the boundary schedule actually applied, independent operator-sign/source/reaction/boundary perturbation failures with recomputed pass/fail thresholds, installed package provenance, and separately hashed request/config/provenance/convention/result/status/evidence identities. `validate_fd_bs_verification_bundle(...)` recomputes all hashes, recomputes `evidence.status`, and reruns the numerical truth before accepting the artifact, so residual convergence, rehashed false rows, or stored booleans alone cannot qualify a run as verified.

Project #17 adds `finite_difference_options.integrations.solve_public_quant_problem_spec` and `released_fd_solver_contract()` as the released public-synthetic FD solver contract API for Pinares fixed-price proxy parity. The API consumes exact checked-in QuantProblemSpec fixtures, preserves measure/numeraire/units/date conventions through `FDRouteRequest`, executes only the validated Black-Scholes/Pinares public-synthetic routes, and accepts an optional `BandedOperatorCache` so repeated 1D solves reuse theta-system operators and pivoted LAPACK factorizations without caching RHS values or boundary data.
...
## Cached tridiagonal LAPACK adapter
