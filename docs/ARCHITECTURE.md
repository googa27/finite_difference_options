...
Operators are constructed from explicit grid and coefficient records. Process objects expose an executable generator contract with a single normalized coefficient convention: `evaluate_coefficients(t, x)` returns batched `states (n,d)`, `drift (n,d)`, `covariance (n,d,d)`, and `discount (n,)` even when the caller supplies one state point. Exact affine covariance, when available, uses the tensor form `a(x,t)=a0(t)+sum_k x_k a_k(t)` through `AffineCovarianceForm`; models whose native covariance is quadratic/bilinear in the declared state fail closed instead of advertising an inexact affine form. Generator application uses `0.5 tr(a Hessian)+b·grad-c value+source`, with discount supplied explicitly and never inferred from drift. Each derivative declares order, local stencil, boundary closure and bias.
