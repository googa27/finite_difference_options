...
Issue #142 adds the executable `fd-bs-001` verification artifact runner at `fd-options validation run-benchmark fd-bs-001 --out <path>`. It reuses the exact compiled Black-Scholes native FD route and emits independently recomputable evidence: analytical q-aware price/Delta/Gamma gates, three-level spatial and temporal refinement tables, payoff/PDE/algebraic/boundary residual norms, manufactured-solution residual order, no-arbitrage checks, the boundary schedule actually applied, independent operator-sign/source/reaction/boundary perturbation failures, installed package provenance, and request/config/convention/result/evidence hashes. `validate_fd_bs_verification_bundle(...)` recomputes all hashes and the numerical truth before accepting the artifact, so residual convergence or stored booleans alone cannot qualify a run as verified.
