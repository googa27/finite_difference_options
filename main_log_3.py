...
def get_bcs(x,
    ...
    if bc_types[0] == 'neumann':
        bc[0] = d1, -s[0]/mkt.D(th)*(not is_call)
    elif bc_types[0] == 'gamma':
        bc[0] = d2, -s[0]/mkt.D(th)*(not is_call)
    ...
    if bc_types[-1] == 'neumann':
        bc[-1] = d1, s[-1]/mkt.D(th)*is_call
    if bc_types[-1] == 'gamma':
        bc[-1] = d2, s[-1]/mkt.D(th)*is_call
...
for i, ti in enumerate(t[:-1]):
    ...
    u_tx[i+1] = pde.solve()
    if is_american:
        u_tx[i + 1] = np.maximum(u_tx[i + 1], phi)
        if is_call:
            s_ex[i + 1] = s[np.argmin(u_tx[i + 1] > phi)]
        else:  # put
            s_ex[i + 1] = s[np.argmax(u_tx[i + 1] > phi)]
...
theta = -mkt.D(t)[:, None]*(Dt(u_tx) - r*u_tx)
# gamma = (r*v_tx - theta - r*s*delta)/(c*s**2)
