...
def validate_state_dimensions(
    state: NDArray[np.float64], expected_dimension: int, process_name: str = "process"
) -> None:
...
def reshape_to_grid(
    flat_result: NDArray[np.float64], grid_shape: tuple, result_dimension: int = 1
) -> NDArray[np.float64]:
...
def batch_state_evaluation(
    state_matrix: NDArray[np.float64], evaluation_func, batch_size: int = 1000
) -> NDArray[np.float64]:
...
def interpolate_state_values(
    ...
    return griddata(state_points, values, query_points, method=method, fill_value=0.0)
